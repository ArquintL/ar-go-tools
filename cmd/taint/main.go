...
var (
	...
	// Other constants
	buildmode = ssa.InstantiateGenerics | ssa.GlobalDebug // necessary for reachability
	version   = "unknown"
)
...
func main() {
	...
	program, packages, err := analysis.LoadProgram(nil, "", buildmode, flag.Args())
	if err != nil {
	...
	start := time.Now()
	result, err := taint.Analyze(taintConfig, program, packages)
